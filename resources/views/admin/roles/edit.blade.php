<x-layout>
    <x-slot name="title">Edit Role - {{ config('app.name') }}</x-slot>
    ...
    <div @class(['flex', 'flex-col', 'items-center', 'justify-center', 'min-h-screen', 'pt-24', 'pb-12', 'px-4', 'sm:px-6', 'lg:px-8', 'bg-background'])>
        <div @class(['max-w-xl', 'w-full', 'space-y-8', 'bg-white', 'p-8', 'md:p-10', 'rounded-3xl', 'shadow-2xl', 'border', 'border-gray-100'])>
            <div @class(['text-center'])>
                <h2 @class(['text-3xl', 'font-bold', 'text-text', 'tracking-tight'])>Edit Role</h2>
                <p @class(['mt-2', 'text-sm', 'text-secondary'])>Update role details and permissions.</p>
            </div>

            <form @class(['mt-8', 'space-y-6']) action="{{ route('admin.roles.update', $role->id) }}" method="POST">
                @csrf
                <div @class(['space-y-6'])>
                    <div @class(['grid', 'grid-cols-1', 'gap-6', 'md:grid-cols-2'])>
                        <div @class(['md:col-span-2'])>
                            <label for="name" @class(['block', 'text-sm', 'font-semibold', 'text-text', 'mb-2'])>Role Name</label>
                            <input id="name" name="name" type="text" value="{{ old('name', $role->name) }}" required @class(['appearance-none', 'block', 'w-full', 'px-4', 'py-3.5', 'border', 'border-gray-200', 'rounded-xl', 'focus:outline-none', 'focus:ring-2', 'focus:ring-primary/20', 'focus:border-primary'])>
                        ...
                        <div @class(['md:col-span-2'])>
                            ...
                            <textarea id="description" name="description" rows="3" @class(['appearance-none', 'block', 'w-full', 'px-4', 'py-3.5', 'border', 'border-gray-200', 'rounded-xl', 'focus:outline-none', 'focus:ring-2', 'focus:ring-primary/20', 'focus:border-primary'])>{{ old('description', $role->description) }}</textarea>
                        ...
                        <div @class(['relative'])>
                            <select id="brand_id" name="brand_id" @class(['appearance-none', 'block', 'w-full', 'px-4', 'py-3.5', 'border', 'border-gray-200', 'rounded-xl', 'focus:outline-none', 'focus:ring-2', 'focus:ring-primary/20', 'focus:border-primary', 'bg-white'])>
                                ...
                                @foreach($brands as $brand)
                                    <option value="{{ $brand->id }}" {{ old('brand_id', $role->brand_id) == $brand->id ? 'selected' : '' }}>{{ $brand->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div @class(['grid', 'grid-cols-1', 'sm:grid-cols-2', 'gap-4'])>
                            ...
                            <label @class(['flex', 'items-start', 'p-3', 'border', 'border-border', 'rounded-xl', 'cursor-pointer', 'hover:bg-slate-50', 'transition-colors', 'has-[:checked]:border-primary/50', 'has-[:checked]:bg-primary/5'])>
                                <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" {{ in_array($permission->id, old('permissions', $role->permissions->pluck('id')->toArray())) ? 'checked' : '' }} @class(['mt-1', 'w-4', 'h-4', 'text-primary', 'border-gray-300', 'rounded', 'focus:ring-primary'])>
                                <div @class(['ml-3'])>
